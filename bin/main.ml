...
let speclist = [
  "-", Arg.Unit (fun () -> file_ic := Some stdin), 
      "Read source from stdin";
  "-ast-dump", Arg.Symbol (["pprint"; "json"; "dot"], fun s -> ast_dump_format := s),
      "Dump the AST in the specified format (pprint, json, dot)";
  "-backend", Arg.Symbol (["shm"; "http"], fun s -> backend := s),
...
|}
  ...
  | "http" ->
    ...
    let executable_names =
      List.map (fun loc -> Printf.sprintf "%s_%s" base_name loc) locs
      |> String.concat " "
    in
...
let () =
  ...
  dump_choreo_ast !ast_dump_format (get_output_path (match !ast_dump_format with
    | "json" -> ".json"
    | "pprint" -> ".ast" 
    | "dot" -> ".dot"
    | _ -> invalid_arg "Invalid ast-dump format")) program;
  ...
  (* Dump network ASTs *)
  List.iter2 (fun loc ir ->
    dump_net_ast !ast_dump_format (get_output_path ("." ^ loc ^ (match !ast_dump_format with
      | "json" -> ".json"
      | "pprint" -> ".ast"
      | _ -> ""))) ir
  ) locs netir_l;
