...
let basic_structure_tests = "Basic Structure Tests" >::: [
...
(* Declaration Tests *)
let declaration_tests = "Declaration Tests" >::: [
...
(* Expression Tests *)
let expression_tests = "Expression Tests" >::: [
...
let product_sum_tests = "Product and Sum Type Tests" >::: [
...
let pattern_matching_tests = "Pattern Matching Tests" >::: [
  "test_parse_match_with_left" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Left(Local.M.Var(Local.M.VarId("z", _), _), _), Net.M.Unit _)], _), _)] -> ()
  ...
  "test_parse_match_with_right" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Right(Local.M.Var(Local.M.VarId("z", _), _), _), Net.M.Unit _)], _), _)] -> ()
  ...
  "test_parse_match_with_pair" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Pair(Local.M.Var(Local.M.VarId("a", _), _), 
                                            Local.M.Var(Local.M.VarId("b", _), _), _), Net.M.Unit _)], _), _)] -> ()
  ...
  "test_parse_match_with_value" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Val(Local.M.Int(42, _), _), Net.M.Unit _)], _), _)] -> ()
  ...
  "test_parse_match_with_default" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Default(_), Net.M.Unit _)], _), _)] -> ()
  ...
  "test_parse_match_with_nested_patterns" >:: (fun _ ->
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Left(Local.M.Pair(
                                  Local.M.Var(Local.M.VarId("a", _), _),
                                  Local.M.Var(Local.M.VarId("b", _), _), _), _), Net.M.Unit _)], _), _)] -> ()
    | _ -> assert_failure "Failed to parse match with nested patterns"
  );

  "test_parse_match_with_multiple_cases" >:: (fun _ ->
    let result = parse_net_string "x := match y with | left z -> unit | right w -> unit | _ -> unit;" in
    match result with
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Match(Net.M.Var(Local.M.VarId("y", _), _), 
                              [(Local.M.Left(Local.M.Var(Local.M.VarId("z", _), _), _), Net.M.Unit _);
                               (Local.M.Right(Local.M.Var(Local.M.VarId("w", _), _), _), Net.M.Unit _);
                               (Local.M.Default(_), Net.M.Unit _)], _), _)] -> ()
...
let network_communication_tests = "Network Communication Tests" >::: [
  ...
  "test_parse_allow_with_multiple_options" >:: (fun _ ->
    let result = parse_net_string "x := allow choice from server with | option1 -> unit | option2 -> unit;" in
    match result with
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.AllowChoice(Local.M.LocId("server", _), 
                                   [(Local.M.LabelId("option1", _), Net.M.Unit _);
                                    (Local.M.LabelId("option2", _), Net.M.Unit _)], _), _)] -> ()
    | _ -> assert_failure "Failed to parse allow choice with multiple options"
  );
]

(* Tests for each grammar production in net_expr *)
let net_expr_tests = "Net Expression Grammar Tests" >::: [
  ...
  "test_var_reference" >:: (fun _ ->
    ...
    match result with
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Var(Local.M.VarId("y", _), _), _)] -> ()
  ...
  "test_allow_expression" >:: (fun _ ->
    let result = parse_net_string "x := allow choice from server with | option -> unit;" in
    match result with
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.AllowChoice(Local.M.LocId("server", _), 
                                   [(Local.M.LabelId("option", _), Net.M.Unit _)], _), _)] -> ()
...
(* Parser Error Tests *)
let parser_error_tests = "Parser Error Tests" >::: [
...
let precedence_tests = "Operator Precedence Tests" >::: [
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Ret(Local.M.BinOp(
                     ...
                     Local.M.BinOp(
                       ...
                       Local.M.Times _,
                       Local.M.Val(Local.M.Int(3, _), _), _), _), _), _)] -> ()
    ...
    | [Net.M.Assign([Local.M.Var(Local.M.VarId("x", _), _)], 
                   Net.M.Ret(Local.M.BinOp(
                     Local.M.BinOp(
                       Local.M.Val(Local.M.Int(1, _), _),
                       Local.M.Plus _,
                       Local.M.Val(Local.M.Int(2, _), _), _),
                     Local.M.Gt _,
                     Local.M.BinOp(
