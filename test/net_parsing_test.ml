...
let declaration_tests =
  ...
  >::: [ ("test_parse_unit_declaration"
       ...
       ; ("test_parse_foreign_declaration"
          >:: fun _ ->
       ...
       ; ("test_parse_complex_types"
          >:: fun _ ->
          let result =
            parse_net_string
              "x : client.int; y : unit -> unit; z : unit * unit; w : unit + unit;"
          in
          match result with
          | [ Net.M.Decl
...
let pattern_matching_tests =
  "Pattern Matching Tests"
  >::: [ ("test_parse_match_with_left"
          >:: fun _ ->
          let result = parse_net_string "x := match y with | left z -> unit;" in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.Match
       ...
       ; ("test_parse_match_with_right"
          >:: fun _ ->
          let result = parse_net_string "x := match y with | right z -> unit;" in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.Match
                    ( Net.M.Var (Local.M.VarId ("y", _), _)
                    , [ ( Local.M.Right (Local.M.Var (Local.M.VarId ("z", _), _), _)
                        , Net.M.Unit _ )
                      ]
                    , _ )
                , _ )
       ...
       ; ("test_parse_match_with_pair"
          >:: fun _ ->
          let result = parse_net_string "x := match y with | (a, b) -> unit;" in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.Match
       ...
       ; ("test_parse_match_with_nested_patterns"
          >:: fun _ ->
          let result = parse_net_string "x := match y with | left (a, b) -> unit;" in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.Match
       ...
       ; ("test_parse_match_with_multiple_cases"
          >:: fun _ ->
          let result =
            parse_net_string
              "x := match y with | left z -> unit | right w -> unit | _ -> unit;"
          in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.Match
                    ...
                    , [ ( Local.M.Left (Local.M.Var (Local.M.VarId ("z", _), _), _)
                        , Net.M.Unit _ )
                      ; ( Local.M.Right (Local.M.Var (Local.M.VarId ("w", _), _), _)
                        , Net.M.Unit _ )
                      ; (Local.M.Default _, Net.M.Unit _)
                      ]
                    , _ )
...
let network_communication_tests =
  ...
  >::: [ ("test_parse_send_recv"
       ...
       ; ("test_parse_choose_allow"
          >:: fun _ ->
          let result =
            parse_net_string
              "x := choose option for client in unit; y := allow choice from server with \
               | option -> unit;"
          in
          match result with
          | [ Net.M.Assign
       ...
       ; ("test_parse_allow_with_multiple_options"
          >:: fun _ ->
          let result =
            parse_net_string
              "x := allow choice from server with | option1 -> unit | option2 -> unit;"
          in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
          ...
          | _ -> assert_failure "Failed to parse allow choice with multiple options")
       ]
;;
...
let net_expr_tests =
  ...
  >::: [ ("test_unit_literal"
       ...
       ; ("test_var_reference"
          ...
          | _ -> assert_failure "Failed to parse variable reference in net_expr")
       ; ("test_ret_expression"
          >:: fun _ ->
          let result = parse_net_string "x := ret 42;" in
          match result with
       ...
       ; ("test_choose_expression"
          >:: fun _ ->
          let result = parse_net_string "x := choose option for client in unit;" in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
                , Net.M.ChooseFor
       ...
       ; ("test_allow_expression"
          >:: fun _ ->
          let result =
            parse_net_string "x := allow choice from server with | option -> unit;"
          in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
       ...
       ; ("test_function_definition"
          ...
          | _ -> assert_failure "Failed to parse function definition in net_expr")
       ; ("test_pair_expression"
          >:: fun _ ->
       ...
       ; ("test_match_expression"
          >:: fun _ ->
          let result =
            parse_net_string "x := match y with | left z -> unit | right w -> unit;"
          in
          match result with
          | [ Net.M.Assign
                ( [ Local.M.Var (Local.M.VarId ("x", _), _) ]
       ...
       ; ("test_parenthesized_expression"
          ...
          | _ -> assert_failure "Failed to parse parenthesized expression in net_expr")
       ]
;;
...
let precedence_tests =
  ...
  >::: [ ("test_arithmetic_precedence"
          ...
          | _ -> assert_failure "Failed to parse arithmetic with correct precedence")
       ; ("test_comparison_precedence"
          ...
          | _ ->
            assert_failure "Failed to parse comparison operations with correct precedence"
         )
...
let additional_token_tests =
  ...
  >::: [ (* Boolean AND operator *)
       ...
       ; (* NOT operator (must be inside ret) *)
         ("test_parse_not_expression"
          ...
          | [ Net.M.Assign
                ...
                , Net.M.Ret
                    ( Local.M.UnOp
                        (Local.M.Not _, Local.M.Val (Local.M.Bool (false, _), _), _)
                    , _ )
                , _ )
            ] -> ()
          | _ -> assert_failure "Failed to parse not expression")
       ; (* String type declaration *)
         ("test_parse_string_type_declaration"
          >:: fun _ ->
       ...
       ; (* Bool type declaration *)
         ("test_parse_bool_type_declaration"
          >:: fun _ ->
       ...
       ; (* String literal assignment *)
         ("test_parse_string_literal_assignment"
          >:: fun _ ->
