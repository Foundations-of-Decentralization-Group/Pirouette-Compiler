...
let test_chan_pair _ =
  ...
  assert_equal true
    (contains_substring result_str "Domainslib.Chan.make_bounded");
...
let test_chan_send _ =
  let send_expr =
    Msg_chan_intf.emit_net_send ~src:"Alice" ~dst:"Bob"
      (Ast_builder.Default.eint ~loc 5)
  in
...
let test_http_send _ =
  let send_expr =
    Msg_http_intf.emit_net_send ~src:"Alice" ~dst:"Bob"
      (Ast_builder.Default.eint ~loc 5)
  in
  let result = expr_to_string send_expr in
