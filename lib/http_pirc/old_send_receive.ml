...
let send_message ~location ~data =
  ...
  | Ok loc_config -> (
      try
        ...
        if status = 200 then Lwt.return_ok ()
        else
          Lwt.return_error
            ("Failed to send message, status code: " ^ string_of_int status)
      with e -> Lwt.return_error ("Send error: " ^ Printexc.to_string e))
      ...
      let server_promises =
        List.map
          (fun loc_cfg ->
            ...
            Hashtbl.replace loc_to_address loc_cfg.Config_parser.location
              new_address;
                  ...
                  (fun () ->
                    let callback _conn _req body =
                      ...
                      if String.length body_string > 0 then (
                                   ...
                                   then
                                     ...
                                     | Ok v ->
                                         if Obj.is_int (Obj.repr v) then
                                           ...
                                           Obj.tag (Obj.repr v) = Obj.string_tag
                                         then "Ok(\"" ^ Obj.magic v ^ "\")"
                                         else "Ok(<value>)"
                                     | Error msg -> "Error(\"" ^ msg ^ "\")"
                                   else "<complex data>"
                        ...
                        Hashtbl.replace message_queues
                          loc_cfg.Config_parser.location body_string;
