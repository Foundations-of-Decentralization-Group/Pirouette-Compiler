...
let send_message ~location ~data =
  ...
  Printf.printf
    "Attempting to send message to location: %s (mapped to: %s)\n"
    location
    actual_location;
  ...
  | Ok loc_config ->
    (try
       ...
       Printf.printf
         "Sending request to: %s with data length: %d\n"
         loc_config.http_address
         (String.length marshaled_data);
...
let receive_message ~location =
  ...
  Printf.printf
    "Attempting to receive message from location: %s (mapped to: %s)\n"
    location
    actual_location;
  match get_location_config actual_location with
  | Error msg ->
  ...
  | Ok loc_config ->
    ...
    if status = 200 && String.length body_str > 0
    then (
      try
    ...
    then Lwt.return_error "Received empty response"
    else Lwt.return_error ("Received error status: " ^ string_of_int status)
;;
