  ...
  let emit_net_send ~src ~dst pexp =
    [%expr
      Domainslib.Chan.send
        [%e Builder.evar (spf "chan_%s_%s" src dst)]
        [%e pexp]]
...
let emit_toplevel_domain out_chan (loc_ids : string list)
      ...
      (fun loc_id net_stmtblock ->
        Builder.value_binding
          ...
          ~expr:
            [%expr
              Domain.spawn (fun _ ->
                  [%e emit_net_toplevel loc_id net_stmtblock])])
      loc_ids net_stmtblocks
  in
  ...
  let emit_chan_defs loc_ids : structure_item list =
    ...
    List.map
      (fun (a, b) ->
        [%stri
          let [%p Builder.pvar (spf "chan_%s_%s" a b)] :
              string Domainslib.Chan.t =
