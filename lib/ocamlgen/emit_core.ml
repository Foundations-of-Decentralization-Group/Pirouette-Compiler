...
let rec emit_local_pexp (expr : 'a Local.expr) =
  ...
  | Val (ListIn (l, _), _) ->
    let build_cons ~loc head tail = [%expr [%e head] :: [%e tail]] in
    let rec return_list input_list =
      match input_list with
      | Local.Nil _ -> [%expr []]
      | Local.Cons (head, rest_of_list, _) ->
        (match head with
         | Int (i, _) -> build_cons ~loc (Builder.eint i) (return_list rest_of_list)
         | String (s, _) -> build_cons ~loc (Builder.estring s) (return_list rest_of_list)
         | Bool (b, _) -> build_cons ~loc (Builder.ebool b) (return_list rest_of_list)
         | _ -> [%expr []])
    in
    return_list l
...
and emit_local_ppat (pat : 'a Local.pattern) =
  ...
  | Val (ListIn (l, _), _) ->
    let rec return_list input_list =
      ...
      | Local.Nil _ -> [%pat? []]
      | Local.Cons (head, rest_of_list, _) ->
        let val_to_pat conv_one =
          match conv_one with
          | Local.Int (i, _) -> Builder.pint i
          | Local.String (s, _) -> Builder.pstring s
          | Local.Bool (b, _) -> Builder.pbool b
          | Local.ListIn (_, _) -> Builder.ppat_any
        in
        [%pat? [%p val_to_pat head] :: [%p return_list rest_of_list]]
    in
    return_list l
  ...
  | ListPat(p,_) ->   let rec return_pat_list input_list =
