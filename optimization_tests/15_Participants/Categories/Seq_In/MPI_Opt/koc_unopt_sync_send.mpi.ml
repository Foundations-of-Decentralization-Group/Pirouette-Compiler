  ...
  | 0 ->
    ...
    let rec broadcast_unopt freq =
      ...
      then (
        ...
        (* print_endline "Reached here, ie the L branch response for the root node"; *)
        ...
        (* print_endline ("This is the required value: " ^ Marshal.from_string val_to_print 0); *)
        ...
        let result_arr = Mpi.gather "y" (loc_to_rank "A") Mpi.comm_world in
        (* print_endline ("Result array size is" ^ Int.to_string (Array.length result_arr)); *)
        (* print_endline "Got the result array"; *)
        (* let val_to_print : string = Array.get result_arr 1 in *)
        (* let val_to_print = Marshal.from_string val_to_print 0 in *)
        Array.set result_arr 0 x;
        (* Array.iter *)
        (*   (fun x -> *)
        (*      Printf.printf "This is the val inside the array %d" (Marshal.from_string x 0)) *)
        (*   result_arr; *)
        (* print_endline ("This is the required value: " ^ string_of_int (val_to_print)); *)
        (* print_endline "Terminate - Unoptimized" *)
      )
    in
    let rec start_time = gettimeofday () in
    let rec _unit_1 = broadcast_unopt 1000 in
