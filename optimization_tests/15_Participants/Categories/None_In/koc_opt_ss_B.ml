...
let () =
  ...
  let process_B =
    ...
    let rec broadcast_opt freq =
      ...
      | "L" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"D" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"E" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 10 in
      ...
      | "R" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"D" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"E" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 9 in
