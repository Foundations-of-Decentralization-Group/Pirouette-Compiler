...
let () =
  ...
  let process_D =
    ...
    let rec broadcast_opt freq =
      ...
      | "L" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"H" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"I" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 10 in
      ...
      | "R" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"H" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"I" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 9 in
