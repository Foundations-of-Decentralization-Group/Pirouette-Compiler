...
let () =
  ...
  let process_G =
    ...
    let rec broadcast_opt freq =
      ...
      | "L" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"N" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"O" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 10 in
      ...
      | "R" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"N" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"O" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 9 in
