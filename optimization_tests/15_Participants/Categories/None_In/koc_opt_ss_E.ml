...
let () =
  ...
  let process_E =
    ...
    let rec broadcast_opt freq =
      ...
      | "L" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"J" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"K" ~data:"L")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 10 in
      ...
      | "R" ->
          (match
             Lwt_main.run (Send_receive.send_message ~location:"J" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          (match
             Lwt_main.run (Send_receive.send_message ~location:"K" ~data:"R")
           with
          | Ok () -> ()
          | Error msg -> failwith ("Send error: " ^ msg));
          let rec x = 9 in
