[@@@warning "-26"]
let (chan_Pinger_Ponger : string Domainslib.Chan.t) =
  Domainslib.Chan.make_bounded 0
let (chan_Ponger_Pinger : string Domainslib.Chan.t) =
  Domainslib.Chan.make_bounded 0
;;let domain_Pinger =
    Domain.spawn
      (fun _ ->
         ...
         let rec pingpong_PIROUETTE_ID (pingCount_PIROUETTE_ID, _) =
           if pingCount_PIROUETTE_ID > 0
           then
             (Domainslib.Chan.send chan_Pinger_Ponger "PING_PIROUETTE_ID";
              (match Domainslib.Chan.recv chan_Ponger_Pinger with
               | "PONG_PIROUETTE_ID" ->
                   let rec newPingCount_PIROUETTE_ID =
                     pingCount_PIROUETTE_ID - 1 in
                   pingpong_PIROUETTE_ID (newPingCount_PIROUETTE_ID, ())
               | _ -> failwith "Runtime Error: Unmatched label"))
           else
             (Domainslib.Chan.send chan_Pinger_Ponger "STOP_PIROUETTE_ID"; ()) in
         let rec main_PIROUETTE_ID = pingpong_PIROUETTE_ID (40000, ()) in ())
  and domain_Ponger =
    Domain.spawn
      (fun _ ->
         let rec print_string_PIROUETTE_ID : ((string) -> (unit)) =
         ...
         let rec stdlib_version_PIROUETTE_ID = () in
         let rec display_stdlib_info_PIROUETTE_ID _ = () in
         let rec pingpong_PIROUETTE_ID (_, pongCount_PIROUETTE_ID) =
           match Domainslib.Chan.recv chan_Pinger_Ponger with
           | "STOP_PIROUETTE_ID" -> ()
           | "PING_PIROUETTE_ID" ->
               let rec newPongCount_PIROUETTE_ID = pongCount_PIROUETTE_ID + 1 in
               (Domainslib.Chan.send chan_Ponger_Pinger "PONG_PIROUETTE_ID";
                pingpong_PIROUETTE_ID ((), newPongCount_PIROUETTE_ID))
           | _ -> failwith "Runtime Error: Unmatched label" in
         let rec main_PIROUETTE_ID = pingpong_PIROUETTE_ID ((), 0) in ()) in
  Domain.join domain_Pinger; Domain.join domain_Ponger
