...
J.test_collatz inp :=
        ...
        then 
            ...
            let J.res1 := J.(inp/2); in 
            let J.res2 := J.(res1 * 2); in
                if J.(res2 = inp) then
                    let J.part_res := J.test_collatz J.(res1); in J.(part_res + 1)
                else 
                    let J.res3 := J.(3 * inp + 1); in
                    let J.res4 := J.test_collatz J.res3; in
                    J.(1 + res4);
...
K.test_collatz inp :=
        ...
        then 
            ...
            let K.res1 := K.(inp/2); in 
            let K.res2 := K.(res1 * 2); in
                if K.(res2 = inp) then
                    let K.part_res := K.test_collatz K.(res1); in K.(part_res + 1)
                else 
                    let K.res3 := K.(3 * inp + 1); in
                    let K.res4 := K.test_collatz K.res3; in
                    K.(1 + res4);
...
L.test_collatz inp :=
        ...
        then 
            ...
            let L.res1 := L.(inp/2); in 
            let L.res2 := L.(res1 * 2); in
                if L.(res2 = inp) then
                    let L.part_res := L.test_collatz L.(res1); in L.(part_res + 1)
                else 
                    let L.res3 := L.(3 * inp + 1); in
                    let L.res4 := L.test_collatz L.res3; in
                    L.(1 + res4);
...
M.test_collatz inp :=
        ...
        then 
            ...
            let M.res1 := M.(inp/2); in 
            let M.res2 := M.(res1 * 2); in
                if M.(res2 = inp) then
                    let M.part_res := M.test_collatz M.(res1); in M.(part_res + 1)
                else 
                    let M.res3 := M.(3 * inp + 1); in
                    let M.res4 := M.test_collatz M.res3; in
                    M.(1 + res4);
...
N.test_collatz inp :=
        ...
        then 
            ...
            let N.res1 := N.(inp/2); in 
            let N.res2 := N.(res1 * 2); in
                if N.(res2 = inp) then
                    let N.part_res := N.test_collatz N.(res1); in N.(part_res + 1)
                else 
                    let N.res3 := N.(3 * inp + 1); in
                    let N.res4 := N.test_collatz N.res3; in
                    N.(1 + res4);
O.test_collatz inp :=
        ...
        then 
            ...
            let O.res1 := O.(inp/2); in 
            let O.res2 := O.(res1 * 2); in
                if O.(res2 = inp) then
                    let O.part_res := O.test_collatz O.(res1); in O.(part_res + 1)
                else 
                    let O.res3 := O.(3 * inp + 1); in
                    let O.res4 := O.test_collatz O.res3; in
                    O.(1 + res4);
...
broadcast_opt freq :=
    ...
    A[L14] ~> O;    
        ...
        let J.result  := J.test_collatz J.931386509544713451; in 
        let A.reply_J := [J] J.result ~> A; in

        let K.result  := K.test_collatz K.931386509544713451; in 
        let A.reply_K := [K] K.result ~> A; in

        let L.result  := L.test_collatz L.931386509544713451; in 
        let A.reply_L := [L] L.result ~> A; in

        let M.result  := M.test_collatz M.931386509544713451; in 
        let A.reply_M := [M] M.result ~> A; in

        let N.result  := N.test_collatz N.931386509544713451; in 
        let A.reply_N := [N] N.result ~> A; in

        let O.result  := O.test_collatz O.931386509544713451; in 
        let A.reply_O := [O] O.result ~> A; in
